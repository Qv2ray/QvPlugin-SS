...
SCENARIO("Test Parse Shadowsocks url", "[ParseSSUrl]")
{
    ...
    GIVEN("A shadowsocks server object")
    {
        ...
        {
            auto c = ss.DeserializeOutbound("ss://YmYtY2ZiOnRlc3Q@192.168.100.1:8888/?plugin=obfs-local%3bobfs%3dhttp%3bobfs-host%3dgoogle.com", &alias,
                                     &err);
            s = ShadowSocksServerObject::fromJson(c.second);
            REQUIRE(s.address.toStdString() == "192.168.100.1");
            REQUIRE(s.port == 8888);
        ...
        {
            auto c =
                ss.DeserializeOutbound("ss://YmYtY2ZiOnRlc3Q@192.168.1.1:8388/?plugin=obfs-local%3bobfs%3dhttp%3bobfs-host%3dgoogle.com", &alias, &err);
            s = ShadowSocksServerObject::fromJson(c.second);
            REQUIRE(s.address.toStdString() == "192.168.1.1");
            REQUIRE(s.port == 8388);
            REQUIRE(s.password.toStdString() == "test");
